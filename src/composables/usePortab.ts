...
export function usePortab() {
  ...
  async function exportConnections(password: string, savePath: string): Promise<PortabResult> {
    ...
    const text = await encryptJson(payload, password);
    await writeTextFile(savePath, text);
    return { count: items.length, warnings };
  }

  /** 从文件导入连接：解密、回写私钥、按 id 合并到列表。 */
  async function importConnections(password: string, filePath: string): Promise<PortabResult> {
    const warnings: string[] = [];
    const text = await readTextFile(filePath);
    const payload = await decryptJson<ExportPayload>(text, password);
